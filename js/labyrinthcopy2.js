//sorok száma: 25
//oszlopok száma: 40  -  ne állítsuk át, a változók 25*40-re vannak beállítva!
//csak 100 sor lehet!
let indexRow = 25;
let indexCol = 40;
...
checking = function (event) {
    ...
    for (let i = 0; i < lengthOfTable; i++) {
        if (numberOfClick % 2 == 0) {
            ...
            // document.querySelectorAll("[title='border']")[i].style["background-color"] = "#0000ff"
        }
        else {
            document.querySelectorAll("[title$='wall']")[i].style["background-color"] = "#000000";
            //document.querySelectorAll("[title='border']")[i].style["background-color"] = "#000000"
        }
    }
    numberOfClick += 1;
};
...
myLastElem = function () {
    ...
    //lastElem["style"]["background-color"] = "#000000";
...
//elkészíti a táblázat sorait és oszlopait
rowId = String("r0");
...
//elkészíti a fekete keretet a táblázat síkja köré
for (let i = 0; i < indexRow; i++) {

    if (i == 0 || i == indexRow - 1) {
        for (let j = 0; j < 9; j++) {
            let myElem = document.querySelector(`#c${i + 1}0${j + 1}`);
            myElem.style["background-color"] = "#000000";
            myElem.title = "borderwall";
            myElem = document.querySelector(`#c${i + 1}0${j + 1}`);
            myElem.style["background-color"] = "#000000";
            myElem.title = "borderwall";
        };
        for (let j = 9; j < indexCol; j++) {
            let myElem = document.querySelector(`#c${i + 1}${j + 1}`);
            myElem.style["background-color"] = "#000000";
            myElem.title = "borderwall";
            myElem = document.querySelector(`#c${i + 1}${j + 1}`);
            myElem.style["background-color"] = "#000000";
            myElem.title = "borderwall";
        };
    }
        ...
        myElem = document.querySelector(`#c${i + 1}01`);
        myElem.style["background-color"] = "#000000";
        myElem.title = "borderwall";
        myElem = document.querySelector(`#c${i + 1}${indexCol}`);
        myElem.style["background-color"] = "#000000";
        myElem.title = "borderwall";
    }
};
...
// az 'indít' gombra klikkeléssel indul a bolyongáa
start = function (event) {

    //a 'repetition' objektum tárolja, hogy a táblázat adott pontjára (key = cellId) hányszor lépett a bolyongás során
    //csak a fekete kereten belül lévő, még üres 'titel'-jű cellák számítanak
    let length = document.querySelectorAll("[title='']").length;
    for (let i = 0; i < length; i++) {
        let key = document.querySelectorAll("[title='']")[i].id;
        key = key.replace("c", "0");
        key = parseInt(key);
        repetition[`${key}`] = 0;
    };

    length = document.querySelectorAll("[title='']").length;
    for (let i = 0; i < length; i++) {
        let key = document.querySelectorAll("[title='']")[i].innerHTML = "";
    };

    
    document.querySelector("#frequency")["disabled"] = "";
    stepCounter = 0;
    goal = false;

    startEvent = event;
    limit = 100000;
    if (startEvent.path[1][1]["value"] != "") {
        limit = startEvent.path[1][1]["value"]
        if (limit > 10000000) {
            alert("Maximum 10^7! A program most 10^7 értékkel indul. Ha végrehajthatatlan a keresés, akkor eltarthat pár percig!");
            limit = 10000000;
            startEvent.path[1][2]["value"] = "10000000"
        }
        //else{limit=}
    };
    ...
    for (let i = 0; i < indexRow * indexCol; i++) {
        let myElem = document.querySelectorAll("td")[i];
        if (myElem.title == "") { myElem.style["background-color"] = "#00ff00" }
    };
    ...
    for (let i = 0; i < limit; i++) {
        ...
        if (document.querySelector(`#c${cellId}`).title != "seeker") { document.querySelector(`#c${cellId}`).title = "" };
        ...
        if (dir == 3) {
            if ((parseInt(cellId / 100) * 100) / 100 < 40) {
                ...
                if (document.querySelector(`#c${cellId + 100}`).title == "target") { goal = true }
            };
        };
        ...
        //if(cellId==targetId-100){console.log("MOST ","cellId: ",cellId, "i: ",i)}
        //if(cellId==targetId-200){console.log("MOST ElŐTTE ","cellId: ",cellId, "i: ",i)}
       ...
       */
        ...
        //console.log("lépés: ", i, " random: ", dir, "previous: ", previousCellId, " last: ", cellId);
        ...
        if (goal == false && stepCounter != limit) { stepCounter = stepCounter + 1 };
        if (goal == false && stepCounter == limit) {
            document.querySelector("#stepCount span").innerHTML =
                `Nem találtam meg a célt ${limit} lépésben!`;
            console.log("previous: ", previousCellId, " last: ", cellId);
        };

        //kiírja a lépésszámot és az eddigi futások min/max lépésszámátt
    ...
    if (myFrequencyFunction == true) { myFrequency() };
};

myFrequency = function () {
    myFrequencyFunction = true;
    document.querySelector("#frequencyText").style.display = "inline";

    length = document.querySelectorAll("[title='']").length;
    for (let i = 0; i < length; i++) {
        let myCell = document.querySelectorAll("[title='']")[i];
        let key = myCell.id;
        key = key.replace("c", "0");
        key = parseInt(key);
        howManyTimes = repetition[`${key}`]
        let howManyTimesDecimal = howManyTimes;
        //0xff0000 = 16711680
        howManyTimes = howManyTimes * 5;
        howManyTimes = howManyTimes + 16711680
        howManyTimes = howManyTimes.toString(16);
        howManyTimes = "#" + howManyTimes;
        if ((howManyTimesDecimal != 0) && (myCell.title != "target")) {
            myCell.style["background-color"] = howManyTimes;
            myCell.style["font-size"] = "10px";
            myCell.innerHTML = howManyTimesDecimal;
        };
    };
}
